...
func TestOrderRepository_Create_HappyPath(t *testing.T) {
	...
	if order.Status != model.OrderStatusPending {
		t.Errorf("status = %v, esperado %v", order.Status, model.OrderStatusPending)
	}
	if order.Total != 100 {
...
func TestOrderRepository_Create_ClienteInexistenteViolaFK(t *testing.T) {
	...
	_, err = repo.Create(context.Background(), tx, model.Order{
...
func TestOrderRepository_UpdateStatus_HappyPath(t *testing.T) {
	...
	updated, err := repo.UpdateStatus(context.Background(), tx, created.ID, model.OrderStatusPaid)
	if err != nil {
	...
	if updated.Status != model.OrderStatusPaid {
		t.Errorf("status = %v, esperado %v", updated.Status, model.OrderStatusPaid)
	}
}
...
func TestOrderRepository_UpdateStatus_NaoEncontrado(t *testing.T) {
	...
	_, err = repo.UpdateStatus(context.Background(), tx, uuid.New(), model.OrderStatusPaid)
