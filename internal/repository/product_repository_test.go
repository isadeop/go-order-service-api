...
func TestProductRepository_Create_NomeDuplicado(t *testing.T) {
	...
	_, err := repo.Create(context.Background(), model.Product{
		Name:  existing.Name,
		Price: 20,
		Stock: 5,
	})
...
func TestProductRepository_Update_HappyPath(t *testing.T) {
	...
	updated, err := repo.Update(context.Background(), tx, created.ID, model.Product{
...
func TestProductRepository_Update_NaoEncontrado(t *testing.T) {
	...
	_, err = repo.Update(context.Background(), tx, uuid.New(), model.Product{
		Name:  "Nome",
		Price: 10,
		Stock: 1,
	})
