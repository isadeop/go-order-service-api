...
type fakeProductRepository struct {
	byName         map[string]model.Product
...
func newFakeProductRepository() *fakeProductRepository {
	return &fakeProductRepository{byName: make(map[string]model.Product)}
}

func (f *fakeProductRepository) Create(ctx context.Context, product model.Product) (model.Product, error) {
	product.ID = uuid.New()
...
func (f *fakeProductRepository) FindAll(ctx context.Context) ([]model.Product, error) {
	products := make([]model.Product, 0, len(f.byName))
...
func (f *fakeProductRepository) FindByID(ctx context.Context, id uuid.UUID) (model.Product, error) {
	for _, p := range f.byName {
		if p.ID == id {
			return p, nil
		}
...
func (f *fakeProductRepository) FindByName(ctx context.Context, name string) (model.Product, error) {
	if f.findByNameErr != nil {
		return model.Product{}, f.findByNameErr
	}
...
func TestProductService_Update_HappyPath(t *testing.T) {
	...
	created, err := f.repo.Create(context.Background(), model.Product{Name: "Notebook", Price: 5000, Stock: 10})
	if err != nil {
